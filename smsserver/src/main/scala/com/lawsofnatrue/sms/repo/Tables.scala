...
trait Tables extends MySQLDBImpl {
  ...
  /** Entity class storing rows of table TmSmsVerifyRecord
    ...
    * @param gmtCreate   Database column gmt_create SqlType(TIMESTAMP)
    * @param gmtUpdate   Database column gmt_update SqlType(TIMESTAMP) */
  case class TmSmsVerifyRecordRow(id: Long, memberId: Long, smsType: Int, content: String, verifyCode: String, channel: Int, traceId: String, ip: Long, deviceType: Byte, fingerPrint: String, status: Byte = 0, verifyTimes: Byte, maxTryTimes: Byte, expireTime: java.sql.Timestamp, resend: Boolean, lastChannel: Int, gmtCreate: java.sql.Timestamp, gmtUpdate: java.sql.Timestamp)
  ...
  implicit def GetResultTmSmsVerifyRecordRow(implicit e0: GR[Long], e1: GR[Int], e2: GR[String], e3: GR[Byte], e4: GR[java.sql.Timestamp], e5: GR[Boolean]): GR[TmSmsVerifyRecordRow] = GR {
    prs => import prs._
      TmSmsVerifyRecordRow.tupled((<<[Long], <<[Long], <<[Int], <<[String], <<[String], <<[Int], <<[String], <<[Long], <<[Byte], <<[String], <<[Byte], <<[Byte], <<[Byte], <<[java.sql.Timestamp], <<[Boolean], <<[Int], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  ...
  class TmSmsVerifyRecord(_tableTag: Tag) extends profile.api.Table[TmSmsVerifyRecordRow](_tableTag, "tm_sms_verify_record") {
    def * = (id, memberId, smsType, content, verifyCode, channel, traceId, ip, deviceType, fingerPrint, status, verifyTimes, maxTryTimes, expireTime, resend, lastChannel, gmtCreate, gmtUpdate) <> (TmSmsVerifyRecordRow.tupled, TmSmsVerifyRecordRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(memberId), Rep.Some(smsType), Rep.Some(content), Rep.Some(verifyCode), Rep.Some(channel), Rep.Some(traceId), Rep.Some(ip), Rep.Some(deviceType), Rep.Some(fingerPrint), Rep.Some(status), Rep.Some(verifyTimes), Rep.Some(maxTryTimes), Rep.Some(expireTime), Rep.Some(resend), Rep.Some(lastChannel), Rep.Some(gmtCreate), Rep.Some(gmtUpdate)).shaped.<>({ r => import r._; _1.map(_ => TmSmsVerifyRecordRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get, _10.get, _11.get, _12.get, _13.get, _14.get, _15.get, _16.get, _17.get, _18.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
